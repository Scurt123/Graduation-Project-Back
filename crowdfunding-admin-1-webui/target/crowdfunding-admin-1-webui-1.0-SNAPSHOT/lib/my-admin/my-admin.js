...
function pageselectCallback(pageIndex, jq) {
    ...
    // 跳转页面
    window.location.href = "admin/query.html?pageNum=" + (pageIndex + 1) + "&keyword=" + window.keyword;
...
function doBatchRemove(adminIdArray) {
    ...
    $.ajax({
        "url": "admin/remove.json",	// 服务器端接收请求的URL地址
        "type": "delete",	// 设置请求方式为POST
        ...
        "success": function (response) {		// 服务器处理请求成功后执行的函数，响应体以参数形式传入当前函数
            ...
            if (result == "SUCCESS") {
                // 跳转页面
                window.location.href = "admin/query.html?pageNum=" + window.pageNum + "&keyword=" + window.keyword;
            }
            if (result == "FAILED") {
