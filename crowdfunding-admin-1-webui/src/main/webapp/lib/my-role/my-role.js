...
function generateTableBody(pageInfo) {
    ...
    if (list == null || list.length == 0) {
        $("#roleTableBody")
            .append("<tr><td colspan='6' style='text-align:center;'>没有查询到数据！</td></tr>");
        return;
    }

    for (var i = 0; i < list.length; i++) {
        var role = list[i];
        var checkBtn = "<button type='button'  roleId='" + role.id + "' class='btn btn-success btn-xs checkBtn'><i class=' glyphicon glyphicon-check' style=\"width: 12px;height: 16px;\"></i></button>";
        var pencilBtn = "<button type='button' roleId='" + role.id + "' class='btn btn-primary btn-xs editBtn'><i class=' glyphicon glyphicon-pencil' style=\"width: 12px;height: 16px;\"></i></button>";
        var removeBtn = "<button type='button' roleId='" + role.id + "' class='btn btn-danger btn-xs removeBtn'><i class=' glyphicon glyphicon-remove' style=\"width: 12px;height: 16px;\"></i></button>";

        var numberTd = "<td width=\"50px\">" + (i + 1) + "</td>";
...
function initPagination(pageInfo) {
    ...
    var paginationProperties = {
        ...
        current_page: window.pageNum > pageInfo.pageNum ? (pageInfo.pageNum - 1) : (window.pageNum - 1),//当前页页码
        prev_text: "上一页",			//上一页文本
        next_text: "下一页"			//下一页文本
    };
...
function getRoleListByRoleIdArray(roleIdArray) {
    ...
    var ajaxResult = $.ajax({
        "url": "role/list.json",
        "type": "POST",
        "data": requestBody,  //用此格式get请求参数传递不过去，不会把json串解析成参数 https://blog.csdn.net/jaryle/article/details/82937256
